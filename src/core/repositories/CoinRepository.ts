...
export class CoinRepository implements ICoinRepository {
  ...
  async findAll(limit = 50, offset = 0): Promise<Coin[]> {
    try {
      ...
      logger.error("No se pudo encontrar todas las monedas", {
        error,
        limit,
        offset,
      });
  ...
  async getAllForComparison(): Promise<Coin[]> {
    ...
    try {
      ...
      logger.error("No se pudo obtener todas las monedas para comparación", {
        error,
      });
      throw new DatabaseError(
        "No se pudo obtener monedas para comparación",
        error,
      );
    }
  }
  ...
  async getImage(id: number): Promise<Buffer | null> {
    try {
      const stmt = await this.db.prepare(
        "SELECT image_data FROM coins WHERE id = ?",
      );
      const row = (await stmt.getAsync(id)) as any;
