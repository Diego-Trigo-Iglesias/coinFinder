...
export class ComparisonService {
    ...
    findBestMatch(
        ...
        for (const coin of existingCoins) {
            ...
            // Verificar si esto es una coincidencia basada en varios criterios
    ...
    findBatchDuplicates(
        ...
        const duplicates: Array<{ primaryIndex: number; duplicateIndex: number; distance: number }> = [];
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            for (let j = i + 1; j < images.length; j++) {
                ...
                const distance = this.calculateHammingDistance(current.hash, other.hash);
        ...
        if (duplicates.length > 0) {
            logger.info('Duplicados por lotes encontrados', { count: duplicates.length });
        }
