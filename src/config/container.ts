    ...
    if (!this._database) {
      ...
      if (isServerless) {
        if (!tursoUrl || tursoUrl.startsWith("file:")) {
          const msg =
            "En entornos serverless (Vercel) se requiere configurar TURSO_DATABASE_URL a una instancia Turso remota. No se puede usar SQLite local (file:) en funciones serverless.";
